...
var NchanPrivateChannel = function (_NchanChannel) {
    inherits(NchanPrivateChannel, _NchanChannel);

    function NchanPrivateChannel() {
        classCallCheck(this, NchanPrivateChannel);
        return possibleConstructorReturn(this, (NchanPrivateChannel.__proto__ || Object.getPrototypeOf(NchanPrivateChannel)).apply(this, arguments));
    }
...
var NchanPresenceChannel = function (_NchanPrivateChannel) {
    inherits(NchanPresenceChannel, _NchanPrivateChannel);

    function NchanPresenceChannel() {
        classCallCheck(this, NchanPresenceChannel);
        return possibleConstructorReturn(this, (NchanPresenceChannel.__proto__ || Object.getPrototypeOf(NchanPresenceChannel)).apply(this, arguments));
    }

    createClass(NchanPresenceChannel, [{
        key: 'here',
...
var NchanConnector = function (_Connector) {
    ...
    createClass(NchanConnector, [{
        ...
        value: function onMessage(message) {
            var parse = message.data.match(/^id: (.*)\n(content-type: (.*)\n)?\n/m);
            var data = JSON.parse(message.data.substr(parse[0].length));
        ...
        key: "addLastMsgIdToQueryString",
        value: function addLastMsgIdToQueryString(url, msgid) {
            if (msgid) {
                var m = url.match(/(\?.*)$/);
        ...
        key: "websocketizeURL",
        value: function websocketizeURL(url) {
            var m = url.match(/^((\w+:)?\/\/([^\/]+))?(\/)?(.*)/);
            var protocol = m[2];
            var host = m[3];
            var absolute = m[4];
...
var Echo = function () {
    function Echo(options) {
        ...
        } else if (this.options.broadcaster == 'nchan') {
            this.connector = new NchanConnector(this.options);
        }
    }
